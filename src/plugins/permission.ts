  ...
  router.beforeEach(
    ...
    > => {
      try {
        ...
        // 设置页面标题
        document.title = getPageTitle(to.meta.title as string | undefined, to)
        ...
        if (apeToken.accessToken) {
          ...
          if (
            ...
            !WHITE_LIST.includes(from.name as string)
          ) {
...
/**
 * 设置路由和用户信息
 * @param {ReturnType<typeof useUserStore>} userStore - 用户状态管理实例
...
const setupRouter = async (
  ...
  try {
    ...
    // 注册动态路由
    routerStore.asyncRouters.forEach((route) =>
...
const handleKeepAlive = async (to: RouteLocationNormalized): Promise<void> => {
  ...
  for (const record of to.matched) {
    const loader = record?.components?.default as any
 ...
 * 处理路由重定向
 * @param {RouteLocationNormalized} to - 目标路由对象
 * @returns {RouteLocationNormalized} 重定向路由对象
 * @description 根据默认路由配置处理重定向逻辑
 */
const handleRedirect = (to: RouteLocationNormalized): any => {
  // 检查默认路由是否存在
