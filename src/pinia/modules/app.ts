...
interface AppConfig {
  ...
  sideMode: MenuNavigationType
  transitionType: string
  cornerSize: BorderRadiusType
...
export interface AppState {
  device: Ref<string>
  drawerSize: Ref<string>
  operateMinWith: Ref<string>
  config: AppConfig
  isDark: Ref<boolean>
}
...
export const useAppStore = defineStore('app', () => {
  ...
  if (sysPreferences) {
    Object.keys(config).forEach((key) => {
      ...
      if (sysPreferences[configKey] !== undefined) {
        const configObj = config as any
        configObj[configKey] = sysPreferences[configKey]
      }
    })
  }
   ...
   * @returns {void}
   */
  const toggleWeakness = (e: boolean): void => {
    config.weakness = e
  }
   ...
   * @returns {void}
   */
  const toggleGrey = (e: boolean): void => {
    config.grey = e
  }
   ...
   * @returns {void}
   */
  const togglePrimaryColor = (e: string): void => {
   ...
   * @returns {void}
   */
  const toggleTabs = (e: boolean): void => {
    config.showTabs = e
  }
   ...
   * @returns {void}
   */
  const toggleDevice = (e: string): void => {
    if (e === 'mobile') {
   ...
   * @returns {void}
   */
  const toggleDarkMode = (e: string): void => {
    config.darkMode = e
  }
   ...
   * @returns {void}
   */
  const toggleConfigSideWidth = (e: number): void => {
   ...
   * @returns {void}
   */
  const toggleConfigSideCollapsedWidth = (e: number): void => {
   ...
   * @returns {void}
   */
  const toggleConfigSideItemHeight = (e: number): void => {
   ...
   * @returns {void}
   */
  const toggleConfigWatermark = (e: boolean): void => {
    config.showWatermark = e
  }

  /**
   * 切换侧边栏模式
   * @param {MenuNavigationType} e - 侧边栏模式
   * @returns {void}
   */
  const toggleSideMode = (e: MenuNavigationType): void => {
    config.sideMode = e
  }
   ...
   * @returns {void}
   */
  const toggleTransition = (e: string): void => {
    config.transitionType = e
  }

  /**
   * 切换圆角大小
   * @param {BorderRadiusType} e - 圆角大小类型
   * @returns {void}
   */
  const toggleCornerSize = (e: BorderRadiusType): void => {
    config.cornerSize = e
  }

  /**
   * 切换组件尺寸
   * @param {ComponentSizeTypeValue} e - 组件尺寸类型
   * @returns {void}
   */
  const toggleComponentSize = (e: ComponentSizeType): void => {
  ...
  const resetConfig = (): void => {
    for (let baseCoinfgKey in defaultConfig) {
      const key = baseCoinfgKey as keyof AppConfig
