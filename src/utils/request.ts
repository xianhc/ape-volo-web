...
let timer: ReturnType<typeof setTimeout> | null = null // 加载延迟定时器
let loadingInstance: LoadingInstance | null = null // Element Plus 加载实例
...
const showLoading = (option: LoadingOption = {}): void => {
  ...
  timer = setTimeout(() => {
    if (activeAxios > 0) {
      const nextOption: LoadingOption = { ...option }
      if (!nextOption.target && loadDom) {
        nextOption.target = loadDom
      }
      const { target, ...restOption } = nextOption
      loadingInstance = ElLoading.service(
        target ? { ...restOption, target } : { ...restOption, fullscreen: true }
      )
    }
  }, 400)
}
...
service.interceptors.request.use(
  (
  ...
  ): CustomAxiosRequestConfig | Promise<never> => {
    // 根据配置决定是否显示加载状态
    if (!config.donNotShowLoading) {
      showLoading(config.loadingOption)
    }

    const isAnonymousApi = anonymousApiList.some((api) =>
    ...
    if (!isAnonymousApi) {
      ...
      const refreshTokenExpires = apeToken.refreshTokenExpires
      ...
      if (apeToken.accessToken && nowTime < apeToken.expires) {
...
service.interceptors.response.use(
  ...
  (
    error: AxiosError<any> & { config: CustomAxiosRequestConfig }
  ): Promise<any> => {
    // 错误响应时关闭加载状态
    if (!error.config?.donNotShowLoading) {
      closeLoading()
    ...
    if (
      ...
      reader.onload = function (): void {
        if (typeof reader.result !== 'string') {
          ElMessage.error(statusMessageMap[status] || '请求失败')
          return
        }
...
let failedQueue: Array<() => void> = [] // 令牌刷新期间失败请求的重试队列
    ...
    if (!isRefreshing) {
      ...
      refreshToken({ token: oldToken.accessToken })
        .then((res) => {
          ...
          const tokenData = ((res.data as { token?: SetTokenData }).token ||
            (res.data as SetTokenData)) as SetTokenData
          setApeToken(tokenData)
